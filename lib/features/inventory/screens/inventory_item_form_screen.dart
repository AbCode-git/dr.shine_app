...
class _InventoryItemFormScreenState extends State<InventoryItemFormScreen> {
  ...
  void initState() {
    ...
    _currentStockController = TextEditingController(text: widget.item?.currentStock.toString());
    _minStockController = TextEditingController(text: widget.item?.minStockLevel.toString() ?? '10.0');
    _reorderController = TextEditingController(text: widget.item?.reorderLevel.toString() ?? '20.0');
    _unitController = TextEditingController(text: widget.item?.unit ?? 'liters');
    _costController = TextEditingController(text: widget.item?.costPerUnit.toString());
    _viscosityController = TextEditingController(text: widget.item?.viscosityGrade);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.item == null ? 'Add Item' : 'Edit ${widget.item!.name}'),
      ),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(child: _buildTextField(_currentStockController, 'Current Stock', Icons.numbers, isNumber: true)),
                  const SizedBox(width: 16),
                  Expanded(child: _buildTextField(_unitController, 'Unit (e.g. Liters)', Icons.straighten)),
                ],
              ),
              const SizedBox(height: AppSizes.p20),
              Row(
                children: [
                  Expanded(child: _buildTextField(_minStockController, 'Min Stock', Icons.warning_amber, isNumber: true)),
                  const SizedBox(width: 16),
                  Expanded(child: _buildTextField(_reorderController, 'Reorder Point', Icons.reorder, isNumber: true)),
                ],
              ),
              const SizedBox(height: AppSizes.p20),
              _buildTextField(_costController, 'Cost Per Unit (ETB)', Icons.payments, isNumber: true),
              if (_category == InventoryCategory.oilChange) ...[
                const SizedBox(height: AppSizes.p20),
                _buildTextField(_viscosityController, 'Viscosity Grade (e.g. 5W-30)', Icons.oil_barrel),
              ],
